...
def get_data(filters):
	...
	# Clean up filter values - remove trailing % characters that may be added by Frappe
	for key in filters:
		if isinstance(filters[key], str):
			filters[key] = filters[key].rstrip('%').strip()
	...
	query = """
		SELECT
			...
			TIMESTAMPDIFF(SECOND, MIN(ec.time), MAX(ec.time)) as original_working_hours,
			0 as late_entry,
			0 as early_exit,
	...
	if filters.get('employee_id'):
		query += " AND ec.employee = %s"

	if filters.get('department'):
		query += " AND e.department = %s"

	query += " GROUP BY DATE(ec.time), ec.employee ORDER BY ec.employee, DATE(ec.time)"
	...
	for record in existing_attendance:
		...
		if key in attendance_status_lookup:
			...
			record['attendance_reference'] = att_record['attendance_reference'] or ''
